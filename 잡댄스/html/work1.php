<!doctype html>
<html lang="ko">
<head>
	<title>동요작가 목일신</title>
	<meta charset="utf-8">
	<meta http-equiv="X-UA-Compatible" content="IE=Edge">
	<meta name="viewport" content="user-scalable=no, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0, width=device-width">
	<link rel="stylesheet" type="text/css" href="../css/jquery.bxslider.css" media="all">
	<link rel="stylesheet" type="text/css" href="../css/swiper.min.css" media="all">
	<link rel="stylesheet" type="text/css" href="../css/front-base.css" media="all">
	<link rel="stylesheet" type="text/css" href="../css/front-style.css" media="all">
	<link rel="stylesheet" type="text/css" href="../css/front-media.css" media="all">
	<link rel="shortcut icon" href="../images/favicon.ico">
	<script src="../js/jquery-1.11.3.min.js" type="text/javascript"></script>
	<script src="../js/pace.js" type="text/javascript"></script>
	<script src="../js/jquery.bxslider.min.js" type="text/javascript"></script>
	<script src="../js/swiper.min.js" type="text/javascript"></script>
	<script src="../js/front-ui.js" type="text/javascript"></script>
</head>
<body>

	<div class="wrap">
		
		<!-- header -->
		<header class="header fixed">
			<div>
				<h1><a href="index.html"><img src="../images/logo.png" alt="잡댄스" style="vertical-align:top;"></a></h1>
				<ul class="gnb">
					<li><a href="birth.php">생애/업적</a></li>
					<li class="gnb__item">
                        <a href="list.php">작품집</a>
                            <ul>
                                <li>
                                    <a style="font-size:12px; font-weight:400;" href="/freelancer/add">프리랜서 지원</a>
                                    <a style="font-size:12px; font-weight:400;" href="/contact/add">업무 제휴 문의</a>
                                    <a style="font-size:12px; font-weight:400;" href="/small/security">기밀 유지 정책</a>
                                </li>
                            </ul>
                    </li>
					<li><a href="movie.php">동영상</a></li>
					<li><a href="introduce.php">목일신 동요제</a></li>
					<li><a href="#">목일신 동시대회</a></li>
					<li class="m-menu login"><a href="#">등록하기</a></li>
					<li class="m-menu"><a href="#">마이페이지</a></li>
					<li class="m-menu"><a href="#">로그아웃</a></li>
				</ul>
				
				<div class="util">
					<a href="#" class="login">등록하기</a>
					<a href="#" class="join">마이페이지</a>
					<a href="#" class="join">로그아웃</a>
				</div>
				
				<a href="javascript:mMenuOn();" class="m-menu-btn">모바일 메뉴 열기</a>
				<a href="javascript:mMenuOff();" class="m-menu-btn_close">모바일 메뉴 닫기</a>
				<a href="javascript:mMenuOff();" class="m-menu-btn_close_x">모바일 메뉴 닫기</a>
			</div>
		</header>
        
<link type="text/css" media="all" href="../css/bootstrap.min.css" rel="stylesheet">
<link type="text/css" media="all" href="../css/mediumish.css" rel="stylesheet">
<link rel="stylesheet" type="text/css" href="../css/base.css" />
<link rel="stylesheet" type="text/css" href="../css/sub.css" />
<link rel="stylesheet" type="text/css" href="../css/board.css" />
<link rel="stylesheet" href="../css/beautiful.css">


<section class="home-hero-cs">
	<div class="animated fadeInUp">
    <h2 class="home-hero-title-cs" style="font-weight:bold;">목일신 전집</h2>
    <p class="home-hero-des-cs">
        work1<br class="br_portfolio">
    </p>
</div>
</section>
<!-- Begin Article
================================================== -->
<div class="container">
	<div class="row">

		<!-- Begin Fixed Left Share -->
		<!-- End Fixed Left Share -->

		<!-- Begin Post -->
		<div style="margin:0 auto;">
			<div class="mainheading" style="margin-top:100px;">

				<!-- Begin Top Meta -->
                <!-- 불필요해보여서 주석처리 함 시작 -->
				<!-- <div class="row post-top-meta">
					<div class="col-md-2">
						<a href="author.html"><img class="author-thumb" src="https://www.gravatar.com/avatar/e56154546cf4be74e393c62d1ae9f9d4?s=250&amp;d=mm&amp;r=x" alt="Sal"></a>
					</div>
					<div class="col-md-10">
						<a class="link-dark" href="author.html"><?=$row->displayName?></a><a href="#" class="btn follow">Follow</a>
						<span class="author-description">Founder of WowThemes.net and creator of <b>"Mediumish"</b> theme that you're currently previewing. Developing professional premium themes, templates, plugins, scripts since 2012.</span>
						<span class="post-date">22 July 2017</span><span class="dot"></span><span class="post-read">6 min read</span>
					</div>
				</div> -->
                <!-- 불필요해보여서 주석처리 함 끝 -->
				<!-- End Top Menta -->

				<h1 class="posttitle">목일신 전집</h1>

			</div>

			<!-- Begin Featured Image -->
			<!-- <img class="featured-image img-fluid" src="/public/subpage/img/demopic/10.jpg" alt=""> -->
			<!-- End Featured Image -->

			<!-- Begin Post Content -->
			<div class="galleryWrap_book_view">
    <div class="thmb">
    	
    	<a href="#" title="새창">
    	
    	
			<img src="../uploadfiles/thumbnail/comm_attach/2015/08/12/63800c1c8e187b0835279cbf329f8f42_500.jpg" style="cursor: pointer;" onclick="viewPopup('9008')"  width="320" height="240" />
		
		
		</a>
    </div>
    <div class="cont">
        <h4 class="titDepth02 dotlB pb15">목일신전집</h4>
        <dl class="mt15">
            <dt class="subDepth02 mb10">발간년도</dt>
            <dd class="mb10">2013.05.10</dd>
            <dt class="subDepth02 mb10">저자</dt>
			<dd class="mb10">목일신</dd>
			<dt class="subDepth02 mb10">출판사</dt>
			<dd  class="mb10">소명출판</dd>
            <dt class="subDepth02 mb10">조회수</dt>
            <dd class="mb10">322</dd>
            <dt class="subDepth02 mb10">페이지</dt>
            <dd class="mb10">495</dd>
        </dl>
		<div class="cont_de_text"><div style="line-height: 1.5;"><span style="font-size: 12pt;">광주전남의 문학사 복원을 위하여 그동안 쌓인 먼지를 터는 두 번째 걸음이다.&nbsp;</span></div><div style="line-height: 1.5;"><span style="font-size: 12pt;">그의 문학적 성과가 규명되기를 바라면서「목일신전집」은 총 3부로 구성하였다.</span></div><div style="line-height: 1.5;"><span style="font-size: 12pt;">1부는 동요와 동시, 2부는 시와 노랫말, 3부는 산문으로 구성하였다.</span></div><div style="line-height: 1.5;"><span style="font-size: 12pt;">부록으로는 작품과 생애연보를 실었다. 이 전집으로 하여 지역문학사 뿐만 아니라 우리의 문학사가 더욱 풍성해지고 촘촘해질 수 있기를 바란다.</span></div></div>
    </div>
    <div class="cb pt20 ofv">
    	
		
		</div>
    </div>
			<!-- End Post Content -->
<!-- Begin Tags -->
            <div class="after-post-tags" style="margin-bottom:150px;">
				<ul class="tags">
					<li><a href="list.php">목록으로</a></li>
				</ul>
			</div>
			<!-- End Tags -->
			

		</div>
		<!-- End Post -->

	</div>
</div>
<!-- End Article
================================================== -->
<!-- Bootstrap core JavaScript
================================================== -->
<!-- Placed at the end of the document so the pages load faster -->
<script src="/public/subpage/js/002_sub/customer/jquery.min.js"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/tether/1.4.0/js/tether.min.js" integrity="sha384-DztdAPBWPRXSA/3eYEEUWrWCy7G5KFbe8fFjk5JAIxUYHKkDx6Qin1DkWx51bBrb" crossorigin="anonymous"></script>
<script src="/public/subpage/js/002_sub/customer/bootstrap.min.js"></script>
<script src="/public/subpage/js/002_sub/customer/ie10-viewport-bug-workaround.js"></script>

        
        
               <div class="footer">
			<ul class="foot-menu">
				<li><a href="#">회사 소개</a></li>
				<li><a href="#">이용 약관</a></li>
				<li><a href="#">개인 보호 정책</a></li>
				<li><a href="#">이메일 주소 무단 수집 거부</a></li>
			</ul>
			
			<ul class="social">
				<li><a href="#"><img src="../images/google.png" alt="구글"></a></li>
				<li><a href="#"><img src="../images/naver.png" alt="네이버"></a></li>
				<li><a href="#"><img src="../images/facebook.png" alt="페이스북"></a></li>
				<li><a href="#"><img src="../images/insta.png" alt="인스타그램"></a></li>
			</ul>
			
			<div class="copy">
				<p>우 59542 전남 고흥군 고흥읍 고흥군청로 1 / 대표전화 061-830-5898 / 팩스 061-830-5577</p>
				<p>&copy; 2015 Goheung Country. ALL RIGHTS RESERVED.</p>
			</div>
		</div>
		<!-- //footer -->
	</div>
	
</body>
</html>

</body>
</html>
