	...
	<form <?=$this->ajax_helper->form("/admin/{$moduleName}/deleteRange"," 복구할 방법이없습니다. 선택한 것을 정말 삭제하시겠습니까?")?>>
			...
			<tbody>
				...
				<?php foreach ( $rows as $row ): ?>
					...
					<tr class="clickable" data-href="<?=my_site_url("/admin/{$moduleName}/get/{$row->id}")?>" >
						...
						<?php endforeach; ?>
						<td class="unclickable ">
							<button type="button"class="btn btn-default clickable "style="height:30px; padding-top:4px;" data-href="<?=my_site_url("/admin/{$moduleName}/update/{$row->id}?type={$row->type}")?>">수정</button>
		...
		<button type="button" class="btn btn-default clickable" data-href="<?=my_site_url("/admin/{$moduleName}/add?type=번역")?>">번역 추가</button>
		<button type="button" class="btn btn-default clickable" data-href="<?=my_site_url("/admin/{$moduleName}/add?type=통역")?>">통역 추가</button>
		<br>
