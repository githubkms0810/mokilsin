...
class Applicationn_M extends Pagination_Model 
{
	...
	{
		$this->db->select("{$this->as}.*,applicant.성명,applicant.성별,applicant.학교,applicant.학년,applicant.연락처");
		...
		$this->db->join("(select application_id, group_concat(학교) as 학교, group_concat(성명) as 성명,group_concat(성별) as 성별, group_concat(학년) as 학년, group_concat(연락처) as 연락처 from applicant GROUP BY application_id) applicant ","{$this->as}.id = applicant.application_id","LEFT");
